...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'landlensdb_dialog_base.ui'))
...
class ConnectionDialog(QtWidgets.QDialog):
    def __init__(self, values, test_callback, parent=None):
        ...
        self.host_input = QtWidgets.QLineEdit(values.get('host', ''))
        self.port_input = QtWidgets.QLineEdit(values.get('port', '5432'))
        self.database_input = QtWidgets.QLineEdit(values.get('database', ''))
...
class LandlensdbDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    STATIC_ROW_ONE = ['SELECT', 'FROM', 'WHERE', 'JOIN', 'LEFT JOIN', 'GROUP BY', 'ORDER BY', 'LIMIT']
    STATIC_ROW_TWO = ['*', 'DISTINCT', 'COUNT(*)', 'SUM()', 'AVG()', 'MIN()', 'MAX()', 'AND', 'OR', 'NOT', 'IN ()', 'LIKE', 'IS NULL', 'IS NOT NULL']
    STATIC_ROW_THREE = ['ST_Intersects()', 'ST_Within()', 'ST_Contains()', 'ST_DWithin()', '::geometry', '::geography']
    ...
    def _load_settings(self):
        ...
        self.connection_values = {
            'name': settings.value('{}/name'.format(self.SETTINGS_PREFIX), ''),
            'service': settings.value('{}/service'.format(self.SETTINGS_PREFIX), ''),
            'host': settings.value('{}/host'.format(self.SETTINGS_PREFIX), ''),
            'port': settings.value('{}/port'.format(self.SETTINGS_PREFIX), '5432'),
            'database': settings.value('{}/database'.format(self.SETTINGS_PREFIX), ''),
            'schema': settings.value('{}/schema'.format(self.SETTINGS_PREFIX), 'public'),
        }
    ...
    def _save_settings(self):
        settings = QtCore.QSettings()
        for key, value in self.connection_values.items():
            settings.setValue('{}/{}'.format(self.SETTINGS_PREFIX, key), value)
    ...
    def _show_error(self, message):
        ...
        if hasattr(self, 'bottom_tabs') and hasattr(self, 'logs_tab'):
    ...
    def _render_dynamic_buttons(self, tables, columns):
        ...
        for layout_name in ('row_four_layout',):
            layout = self._layout_for_name(layout_name)
            if layout.count() > 1 and layout.itemAt(0).widget() and layout.itemAt(0).widget().text().startswith('No '):
                layout.itemAt(0).widget().setEnabled(False)
    ...
    def _connection_kwargs(self, values=None):
        values = values or self.connection_values
        connection_kwargs = {'dbname': values.get('database', '').strip()}
        service = values.get('service', '').strip()
        if service:
            connection_kwargs['service'] = service
        else:
            connection_kwargs['host'] = values.get('host', '').strip()
            connection_kwargs['port'] = values.get('port', '').strip() or '5432'
        return connection_kwargs

    def _test_connection_values(self, values):
        valid, message = self._validate_connection_values(values)
        if not valid:
            return False, message

        try:
            with psycopg2.connect(**self._connection_kwargs(values)) as connection:
    ...
    def add_last_query_to_map(self):
        ...
        group = self._ensure_query_group(query_name)
        added_layers = []
        ...
        for raster_column, row_filters in self._last_query_state['raster_row_map'].items():
            for row_filter, row_label in row_filters:
                raster_layer = self._create_raster_layer(
                    self._last_query_state['raster_source'],
                    raster_column,
                    row_filter,
                    '{} {} {}'.format(query_name, raster_column, row_label),
                )
