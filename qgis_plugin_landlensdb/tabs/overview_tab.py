...
class OverviewTab(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        content.setHtml(
            ...
            <p>Thumbnails use <code>"enabled": "source"</code> to resize the source image or <code>"enabled": "sidecar"</code> to resize a browse image. Browse thumbnails use <code>thumbnail.sidecar_path</code>, defaulting to <code>./{base}-BROWSE.JPG</code>, with the same relative-path rules. Missing browse files leave thumbnails empty. Both modes accept width, height, and resampling; <code>false</code> disables thumbnails. The WorldView template uses browse thumbnails.</p>
